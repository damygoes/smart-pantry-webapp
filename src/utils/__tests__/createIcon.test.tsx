...
describe('createIcon', () => {
  ...
  it('should forward custom props to the IconComponent', () => {
    ...
    const { getByTestId } = render(
      <WrappedIcon size={20} className="custom-class" data-custom="custom-data" />
    );
  ...
  it('should throw an error when IconComponent is undefined or null', () => {
    ...
    expect(() => createIcon(undefined as any)).toThrow('IconComponent is required');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect(() => createIcon(null as any)).toThrow('IconComponent is required');
  });
});
